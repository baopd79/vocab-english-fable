...
- Import từ vựng cho user: upload file `.apkg` (Anki — đọc SQLite trong zip, map field theo note type) hoặc CSV/XLSX theo template (user/chủ app soạn bằng AI rồi import, không tốn quota Gemini khi field đầy đủ — ý tưởng 2026-07-14, từ pipeline convert apkg của starter decks)
...
3. **Starter decks:** nguồn nội dung (Oxford 3000 theo chủ đề?), số deck/số từ, seed bằng management command hay fixture, enrich seed tính vào quota ai. → **Đã chốt (2026-07-14): nguồn = bộ Anki "4000 Essential English Words – Book 1" của user (600 từ, 30 unit × 20), convert offline** bằng script đọc SQLite trong `.apkg` (file nguồn để ở `content/`, gitignored). **Tránh bản quyền sách:** chỉ lấy word list + nghĩa VI ngắn (Short Vietnamese) + IPA + từ loại; định nghĩa/ví dụ nguyên văn sách bỏ — **ví dụ EN/VI do Claude tự sinh** lúc convert (nội dung gốc, không tốn call Gemini nào; media Anki bỏ, TTS thay). Gộp **1 deck 600 từ**; seed = `manage.py seed_starter_decks` đọc JSON commit trong repo, idempotent. Import .apkg/CSV cho end-user → roadmap (mục 15).
4. **Deck công khai:** trang share có cần login để xem không; clone giới hạn số lần? → **Đã chốt (2026-07-17): xem tự do không cần login** (đúng tinh thần chia sẻ; endpoint public có throttle chống scrape), **clone cần login, không giới hạn số lần** (mỗi user vốn chỉ clone được 1 bản do unique tên deck → 409). URL share dùng **id thật** (`/share/{id}` — đã "công khai" thì dò id cũng chỉ thấy deck public; private vẫn 404). Trang share hiện **display name chủ deck** (không bao giờ lộ email), danh sách từ **chỉ content** (word/IPA/nghĩa/ví dụ — không lộ SRS/tiến độ của chủ deck). UI: nút "Chia sẻ" ở trang deck detail → panel toggle Công khai + copy link; clone tái dùng cơ chế T15 (không copy SRS, không tốn lượt AI).
...
6. **P3:** Gemini free tier có chịu nổi seed starter decks + user lạ không → nhìn số liệu usage thật rồi quyết. → **Đã chốt (2026-07-14): ở lại free tier** (gemini-2.5-flash: RPM 5 / TPM 250k / RPD 20; seed starter decks không cần Gemini — xem Q3; bật billing khi có user thật). **Trần toàn hệ thống:** counter trong Django cache (Redis) key theo **ngày Pacific** (giờ reset quota của Google), env `GEMINI_DAILY_BUDGET` (default 18, chừa đệm lệch counter); chỉ đếm call thật — cache hit và FakeProvider không tính. Worker giữ chỗ **trước** khi gọi (incr-then-compare, race-safe); hết trần → fail-as-miss, không retry trong ngày. Request-time (thêm từ / đổi word_text / retry) chặn sớm 429 `code=ai_budget_exceeded` khi từ chưa có cache completed — FE hiện thông báo thân thiện. RPM: Celery `rate_limit` qua env `ENRICHMENT_RATE_LIMIT` (default 4/m). Quota per-user 50/ngày giữ nguyên.
