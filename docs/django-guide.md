...
### Task 21 — Deploy workflow + HTTPS + backup (vocabun.com sống)
- Chuỗi tự động hoàn chỉnh: push `main` → CI (test) → `deploy.yml` build 2 image lên GHCR → SSH vào VPS `git pull` + `compose pull` + migrate one-shot + `up -d` + curl health. VPS: DigitalOcean Singapore 1GB (thêm 1GB swap, gunicorn hạ còn 2 workers).
- Khái niệm mới (task này ít Django, nhiều ops):
  - **`workflow_run` để xâu chuỗi workflow:** Deploy khai báo `on: workflow_run: workflows: [CI]` + `if: conclusion == 'success'` — chỉ deploy khi test xanh, thay vì 2 workflow chạy đua nhau trên cùng một push.
  - **Ba loại credential, mỗi cái một việc, quyền tối thiểu:** deploy key read-only (VPS đọc repo private) ≠ SSH key riêng cho CI nằm trong GitHub Secrets (không dùng key cá nhân) ≠ PAT `read:packages` (VPS pull image GHCR private — `GITHUB_TOKEN` chỉ sống trong Actions, không dùng được từ VPS). Client id Google nằm trong repo **variable** chứ không phải secret vì bản chất nó public (bị nướng vào bundle JS).
  - **Compose overlay:** `-f file1 -f file2` merge theo luật — `ports` nối thêm, `volumes` đè theo target path trong container. Nhờ vậy `docker-compose.tls.yml` chỉ ~10 dòng biến stack http (test local) thành https (VPS) mà không fork file compose.
  - **certbot standalone + pre/post-hook:** hook của certbot phải là *executable trong PATH*, không nhận shell builtin (`cd` fail) → viết 2 script `/usr/local/bin/vocab-nginx-{stop,start}`. Trade-off đã chọn: renew mỗi ~60 ngày chịu ~15s downtime, đổi lấy setup đơn giản hơn hẳn webroot.
  - **Django sau HTTPS thật:** `SECURE_PROXY_SSL_HEADER` phát huy tác dụng — nginx gửi `X-Forwarded-Proto: https` → `request.is_secure()` đúng → refresh cookie mang cờ `Secure`. Redirect 80→443 là việc của nginx (`return 301`), Django không đụng vào.
...
### v1.1 Task 10 — Quick-add từ ở header (Q2: nút "+" header + modal + chips)
- Nút "+" tròn trong header (ngoài nav nên mobile vẫn thấy) mở modal thêm từ từ mọi trang: input + chips chọn deck (deck dùng gần nhất chọn sẵn, nhớ localStorage) + lỗi hiện ngay trong modal. Modal giữ mở sau mỗi lần thêm — input tự clear + focus lại, danh sách "Vừa thêm" hiện kết quả AI live; enrichment chạy như flow thường. Card deck ở /decks có nút "+ Từ" mở cùng modal (deck chọn sẵn). Cùng đợt: khung layout 1080→1280px, deck grid 3 cột desktop. Task frontend thuần.
- Khái niệm mới:
  - **Modal tự viết, không cần library:** overlay `fixed inset-0` bắt click để đóng, panel `stopPropagation()`; Esc + khóa scroll body qua `useEffect` có cleanup; `autoFocus` vào input. Z-index xếp tầng có chủ đích: header `z-10` < confetti `z-20` < modal `z-30` < toast `z-40`.
  - **Gotcha CSS: `backdrop-filter` trên ancestor tạo containing block cho `position: fixed`** (giống `transform`/`filter`). Modal render trong header (có `backdrop-blur-xl`) nên `fixed inset-0` neo vào dải header chứ không phải viewport → panel tràn khỏi màn hình, không click/nhập được. Fix chuẩn: `createPortal(modal, document.body)` — an toàn vì modal chỉ mount sau click (client-side, không đụng SSR); RTL query mặc định quét cả `document.body` nên test không phải đổi.
  - **List "Vừa thêm" gần như miễn phí nhờ cache:** `useAddWord.onSuccess` đã `setQueryData(["word", id], created)` sẵn — nên `AddedWordRow` chỉ cần gọi `useWord(id)` là có data ngay từ cache và *tự poll 2s* khi enrichment còn pending (đúng pattern `WordRow` trang deck đang dùng). Không thêm state, không thêm API.
  - **React 19 ref-as-prop:** `InputHTMLAttributes` không có `ref` nên `<Input ref=…>` không compile; đổi type sang `ComponentProps<"input">` là ref xuyên qua `{...props}` xuống `<input>` thật (React 19 bỏ forwardRef). Cần cho việc refocus input sau mỗi lần thêm để gõ từ tiếp liền tay.
  - **"Derive, đừng sync":** id deck từ localStorage chỉ là *gợi ý* — `selectedId` được suy ra mỗi render từ (pickedId, danh sách deck thật); deck bị xóa/của account khác thì tự rơi về deck đầu, không cần effect đồng bộ.
...
### v1.1 Task 12 — Cram mode: ôn tự do không đụng SM-2 (F4)
- Nút "Ôn tự do ⚡" trên trang deck → route mới `/decks/[id]/cram`: cả deck (từ đã enrich xong) xáo trộn, gõ lại từ → lật → tự chấm "Chưa nhớ"/"Đã nhớ" (Chưa nhớ = gặp lại cuối phiên). **Không có dòng backend nào mới** — phiên chỉ GET danh sách từ sẵn có; không gọi answer API, không ReviewLog, SRS fields nguyên vẹn (AC §17.2-11).
- Khái niệm mới:
  - **AC "DB không đổi" chứng minh bằng test frontend:** không thể test "DB không đổi" ở backend khi backend không có code mới — thay vào đó test cram chạy trọn phiên (cả nhánh requeue) rồi assert **mọi fetch call đều là GET**. GET không ghi DB là invariant sẵn có của backend, nên "chỉ GET" ⇒ "DB không đổi".
  - **Phân trang phía client:** words endpoint phân trang (50/trang, `page_size` max 100 — xem `apps/common/pagination.py`); `fetchAllDeckWords` lặp `?page=n&page_size=100` đến khi `next=null` rồi nối kết quả. Test dựng 2 trang để khẳng định vòng lặp.
  - **Shuffle trong `queryFn`, không trong render:** lint `react-hooks/purity` cấm `Math.random()` khi render (bài học Task 9) — xáo trộn nằm trong queryFn của TanStack Query; "Ôn lại lượt nữa" = `refetch()` → tự có thứ tự mới, kèm `key={round}` remount runner (pattern giống review).
  - **Discriminated union props:** `ReviewCard` thêm `mode: "cram"` — cùng một component, `onGrade` đổi kiểu theo mode (`Rating` vs `CramRating`) nhờ union phân biệt bằng field `mode`; bảng GRADES thành data (4 nút SM-2 vs 2 nút local) thay vì if/else rải rác. `MuteToggle` tách ra `ui/mute-toggle.tsx` dùng chung 2 runner.
  - **Gotcha phím tắt + autoFocus:** handler keydown chấm thẻ mà không `preventDefault()` → thẻ sau mount với input autoFocus *ngay trong lúc event còn chạy*, hành vi mặc định của phím "1"/"2" chèn ký tự vào input mới. Review SM-2 không lộ vì `await submit` đẩy việc đổi thẻ sang microtask sau event; cram chấm đồng bộ nên lộ ngay. Fix: shortcut đã nhận thì luôn `event.preventDefault()`. Test regression: `fireEvent.keyDown` trả `false` khi default bị chặn.
- Đọc: `frontend/src/lib/cram.ts`, `frontend/src/app/decks/[id]/cram/page.tsx` (+ `cram.test.tsx` — test GET-only), `frontend/src/components/review-card.tsx` (union props).

### v1.1 Task 13 — Dạng ôn mới: MCQ + nghe-gõ, phần backend (Q1, §17.2-10)
- Chốt Q1 (quyết định lớn nhất v1.1): thêm 2 dạng ôn **trắc nghiệm (MCQ)** và **nghe-gõ**, dùng chung lịch SM-2 của từ — dạng ôn chỉ quyết định *cách hỏi*, không có lịch riêng. Thẻ mới/non (reps < 2) luôn ôn dạng chuẩn; từ rep 2 xoay vòng tất định `repetitions % 3` (0→MCQ, 1→nghe-gõ, 2→chuẩn). Backend: queue trả thêm `review_mode` + `mcq_choices` (4 nghĩa VI đã xáo server-side), `/review/answer` nhận thêm `mode` ghi vào `ReviewLog`.
- Khái niệm mới:
  - **`TextChoices` + migration có default trên bảng có data:** `ReviewLog.Mode` là enum kiểu Django (`models.TextChoices`); thêm cột `mode` với `default=Mode.CLASSIC` sinh migration `AddField` — Postgres backfill toàn bộ dòng cũ thành `'classic'` ngay trong `ALTER TABLE` (PG11+ làm nhanh, không rewrite bảng), xong Django *bỏ* DB-level default vì default của Django sống ở tầng app. Đã kiểm chứng trên **bản sao data prod** theo restore-runbook: restore dump vào DB tạm → `DATABASE_URL=... manage.py migrate` → 18 log cũ đều `mode='classic'`, cột NOT NULL → drop DB tạm + xóa dump (chứa PII thật).
  - **Annotate attribute lên ORM instance thay vì wrapper:** selector `_assign_modes` gắn thẳng `card.review_mode`/`card.mcq_choices` lên từng `UserWord` trong queue — instance Python gắn attribute tùy ý được, DRF serializer đọc attribute nên chỉ cần khai báo field, và 15+ test selector cũ khỏi phải viết lại vì queue vẫn là list `UserWord`.
  - **Kế thừa serializer:** `ReviewQueueItemSerializer(UserWordSerializer)` + `Meta(UserWordSerializer.Meta)` với `fields = [*cha, "review_mode", "mcq_choices"]` — chỉ nhóm due/new của queue có 2 field mới, còn mọi endpoint khác dùng `UserWordSerializer` giữ nguyên contract.
  - **`ChoiceField(default=...)` cho input:** field `mode` optional, thiếu thì mặc định `classic` — client cũ (chưa deploy Task 14) gửi answer không có `mode` vẫn chạy đúng; giá trị lạ (`"cloze"`) bị 400 ngay ở serializer.
  - **Random ở server, một query cho cả queue:** pool nghĩa VI (distinct, đã enrich) query *lazy* — chỉ chạy khi queue thực sự có thẻ MCQ, và chạy đúng 1 lần dùng chung; `random.sample` + `random.shuffle` xáo đáp án server-side nên frontend không cần randomness lúc render (né lint `react-hooks/purity` từ đầu). Thiếu distractor (< 3 nghĩa khác) → rơi về dạng chuẩn, không bao giờ trả câu hỏi khuyết.
- Đọc: `apps/srs/models.py` (`ReviewLog.Mode`), `apps/srs/selectors.py` (`review_mode`, `_assign_modes`, `_mcq_choices`), `apps/srs/serializers.py` (`ReviewQueueItemSerializer`), `apps/srs/migrations/0002_reviewlog_mode.py`, `apps/srs/tests/test_queue_selector.py` (block review modes).

### v1.1 Task 14 — Dạng ôn mới: MCQ + nghe-gõ, phần frontend (§17.2-10)
- Runner ôn tập giờ hỏi theo `review_mode` backend gán cho từng thẻ: **MCQ** = hiện từ EN + 4 nghĩa VI (bấm hoặc phím 1–4), **nghe-gõ** = TTS tự phát từ (kèm nút nghe lại), gõ lại từ vừa nghe. Cả 2 sau đó lật thẻ đầy đủ + tự chấm 4 nút SM-2 như dạng chuẩn; `mode` gửi kèm POST `/review/answer`. Task frontend thuần.
- Pattern đáng nhớ:
  - **Server quyết định, client chỉ render:** FE không tự tính dạng ôn hay xáo đáp án — `review_mode`/`mcq_choices` đến từ queue API, component chỉ switch theo. Kèm fallback phòng thủ (MCQ thiếu choices → render dạng gõ) dù backend đã đảm bảo, để mặt thẻ không bao giờ trống.
  - **Field optional trên type dùng chung:** `UserWord` thêm `review_mode?`/`mcq_choices?` chỉ tồn tại trên item của `/review/queue` — đánh đổi so với tách type `QueueItem` riêng: gọn, không đổi chữ ký hàng loạt, nhưng phải nhớ các endpoint khác không có 2 field này (docstring nói rõ).
  - **2 lớp phím tắt theo phase, cùng bộ phím:** 1–4 chọn đáp án khi đang ở mặt hỏi MCQ, 1–4 chấm điểm khi đã lật — không đụng nhau vì mỗi effect chỉ gắn listener đúng phase của nó; cả 2 đều `preventDefault()` (bài học Task 12).
  - **Auto-play TTS mount-per-card:** runner `key={step}` nên mỗi thẻ là một lần mount → `useEffect` phát từ đúng 1 lần; mount luôn xảy ra ngay sau một cú click (Bắt đầu ôn / nút chấm) nên còn user activation, không sợ autoplay policy — vẫn thuộc diện smoke trên browser thật.
  - **`vi.mock` module TTS:** jsdom không có `speechSynthesis` (lib `tts.ts` no-op) nên muốn assert "thẻ nghe-gõ tự phát âm" phải mock cả module `@/lib/tts` (hoisted toàn file) rồi `expect(speak).toHaveBeenCalledWith(...)`.
- Đọc: `frontend/src/components/review-card.tsx` (FRONT_LABELS + 3 variant mặt hỏi), `frontend/src/components/review-card.test.tsx` (block review modes), `frontend/src/app/review/review.test.tsx` (phiên MCQ + nghe-gõ end-to-end), `frontend/src/lib/review.ts` (`useSubmitAnswer` mang `mode`).

### v1.1 Task 6 — Trần chi phí Gemini toàn hệ thống (P3, Q6)
- Chốt Q6 từ số liệu console thật: ở lại free tier (RPM 5 / RPD 20). Trần mới chặn ở 2 tầng: **worker** (giữ chỗ budget trước khi gọi Gemini thật, hết trần → fail-as-miss không retry) và **request-time** (thêm từ / đổi word_text / retry khi từ chưa có cache → 429 `ai_budget_exceeded` ngay, FE hiện thông báo thân thiện). Cache hit và FakeProvider không tốn budget. Kèm `rate_limit` Celery 4/m (RPM 5 chừa 1).
- Khái niệm mới:
  - **Django cache API làm counter phân tán:** `cache.add(key, 0, ttl)` + `cache.incr(key)` — trên Redis, `INCR` nguyên tử nên **incr-then-compare** race-safe (2 worker giành slot cuối không thể cùng thắng); LocMemCache trong test cũng hỗ trợ nên không cần Redis khi pytest. Bài học kèm: LocMemCache **sống xuyên test** (DB rollback nhưng cache thì không) → fixture `autouse` `cache.clear()` kẻo counter rò giữa các test.
  - **Key theo "ngày của nhà cung cấp":** quota Google reset lúc nửa đêm **Pacific** — counter key theo `America/Los_Angeles` date (cùng trick ZoneInfo như queue/streak theo timezone user). Mất counter khi Redis restart chỉ có thể *vượt* trần → Google trả 429 → nhánh failed-as-miss sẵn có đỡ.
  - **Protocol attribute làm feature flag:** thêm `is_metered: bool` vào `AIProvider` Protocol — Gemini `True`, Fake `False`; service chỉ hỏi attribute, không `isinstance`. Đi kèm `provider_is_metered()` đọc từ settings cho request-time (không dựng `genai.Client` mỗi POST). Bẫy test: `mock.Mock()` tự sinh attribute **truthy** — phải set `is_metered=False` tường minh trong helper kẻo mọi test cũ lặng lẽ đi vào nhánh budget.
  - **Chặn sớm ở service, không phải view:** `_require_ai_budget()` gọi trong 3 service write (create/rename/retry) — view không đổi dòng nào, đúng layering §10; exception `AIBudgetExhausted(APIException, 429, default_code)` đi qua error contract chung nên FE chỉ thêm 1 nhánh `code === "ai_budget_exceeded"`.
  - **`rate_limit` của Celery đọc settings lúc import:** `@shared_task(rate_limit=settings.ENRICHMENT_RATE_LIMIT)` — đổi qua env không cần sửa code, nhưng `override_settings` trong test không có tác dụng (decorator đã chạy); test eager bỏ qua rate_limit nên vô hại.
- Đọc: `apps/enrichment/budget.py`, `apps/enrichment/tests/test_budget.py`, `apps/enrichment/services.py` (nhánh BUDGET_EXCEEDED), `apps/vocab/services.py` (`_require_ai_budget`), `apps/vocab/tests/test_word_api.py` (block system-wide AI budget).

### v1.1 Task 15 — Starter decks: convert Anki + seed + clone (F1, Q3)
- Chốt Q3: nguồn = bộ Anki "4000 Essential Words – Book 1" của user (600 từ), convert offline; né bản quyền sách bằng cách chỉ lấy từ + nghĩa VI ngắn + IPA + từ loại, còn 600 cặp ví dụ EN-VI do Claude tự soạn. Backend: Deck thêm `is_starter` + `source_deck` (FK self), system user sở hữu deck mẫu, `GET /decks/starter` + `POST /decks/{id}/clone`; seed bằng management command. FE: section "Deck mẫu" trên /decks, clone xong tự ẩn.
- Khái niệm mới:
  - **Management command đầu tiên của project:** `management/commands/seed_starter_decks.py` kế thừa `BaseCommand`, logic write vẫn nằm ở `services.seed_starter_deck` (command chỉ đọc JSON + gọi service — đúng layering §10, test service không cần call_command). Chạy bằng `manage.py seed_starter_decks`, idempotent để chạy lại thoải mái khi thêm content.
  - **`bulk_create`:** 600 UserWord = 1 câu INSERT thay vì 600 — cả seed lẫn clone đều dùng; đổi lại không gọi `save()`/signal từng row (ở đây không cần).
  - **FK trỏ vào chính bảng (`"self"`):** `source_deck` với `on_delete=SET_NULL` — deck gốc xóa thì bản clone vẫn sống, chỉ mất "gia phả". FE dùng chính field này để ẩn deck mẫu đã clone (so `source_deck` với id deck mẫu — không cần API riêng "đã clone chưa").
  - **System user:** row `User` với `is_active=False`, không `google_sub` → không bao giờ login được nhưng vẫn own được deck; tránh phải viết model riêng cho "nội dung hệ thống", mọi serializer/query sẵn có dùng lại nguyên vẹn.
  - **Seed cả WordCache (provider="seed"):** 600 từ phổ biến nhất nằm sẵn trong cache toàn cục → user nào tự gõ "afraid" cũng cache hit, không tốn call Gemini (quý khi RPD=20). Cache có sẵn thì giữ nguyên — cùng triết lý "không sửa WordCache khi user edit".
  - **Clone = copy content, reset SRS:** bulk_create bản sao với `enrichment_status` giữ nguyên nhưng SRS fields để default → mọi từ vào queue như thẻ mới (AC §17.2-13 "không copy tiến độ"); không enqueue task nào nên không đụng quota/budget (AC).
  - **Pipeline content offline:** `.apkg` = zip chứa SQLite (`collection.anki2`, bảng `notes`, field ngăn bằng `\x1f`) — script stdlib-only đọc thẳng, map field theo note type; media bỏ (TTS thay). File nguồn + trung gian nằm ở `content/` (gitignored), chỉ JSON cuối cùng commit vào `apps/vocab/data/`.
- Đọc: `scripts/apkg_to_starter_json.py`, `apps/vocab/services.py` (`clone_deck`, `seed_starter_deck`), `apps/vocab/management/commands/seed_starter_decks.py`, `apps/vocab/tests/test_starter_decks.py`, `frontend/src/app/decks/page.tsx` (`StarterDecksSection`).

### v1.1 Task 16 — Deck công khai / chia sẻ (F2, Q4)
- Chốt Q4: trang share (`/share/{id}`) xem tự do không cần login, clone cần login, không giới hạn số lần; hiện display name chủ deck (không bao giờ lộ email). Backend: `Visibility` mở thêm `public`, owner bật/tắt qua PATCH deck sẵn có; 2 endpoint ẩn danh `GET /decks/{id}/public` + `/public/words`; clone mở rộng cho deck public. FE: panel "Chia sẻ" ở deck detail (toggle + copy link) + trang share đứng ngoài `RequireAuth`.
- Khái niệm mới:
  - **Authentication vs Permission là 2 tầng tách biệt:** endpoint public cần cả `permission_classes = [AllowAny]` (mặc định project là `IsAuthenticated`) **và** `authentication_classes = []`. Bỏ authenticator không phải cho vui: nếu còn JWTAuthentication, một access token hết hạn trong header sẽ 401 cả trang public — client đã login mở share link sau 15 phút sẽ thấy lỗi vô lý. Không authenticator → mọi request là AnonymousUser, trang luôn mở.
  - **Throttle ẩn danh theo IP:** `ScopedRateThrottle` với scope `share` — không có user thì DRF tự key theo IP client (đúng IP thật nhờ `NUM_PROXIES = 1` đã cấu hình từ trước). Nhớ thêm scope vào cả `test.py` (`"share": None`) vì view đặt `throttle_classes` tường minh, không bị tắt theo `DEFAULT_THROTTLE_CLASSES = []`.
  - **Nới `read_only_fields` = mở field cho client ghi:** chỉ cần bỏ `visibility` khỏi danh sách read-only của `DeckSerializer` — ModelSerializer tự sinh ChoiceField từ `choices` của model nên `"unlisted"` bị 400 miễn phí, không viết validator nào. Đầu kia, service `create_deck`/`update_deck` phải nhận kwarg mới (serializer truyền `**validated_data` — thiếu param là TypeError 500).
  - **Đổi `choices` vẫn sinh migration:** thêm `PUBLIC` vào TextChoices → `AlterField` migration nhưng SQL thực tế là no-op (Postgres không biết gì về choices — chúng chỉ được validate ở tầng Python/serializer).
  - **Serializer riêng cho "góc nhìn public":** `PublicDeckSerializer`/`PublicWordSerializer` chỉ expose content — không SRS, không enrichment_status, không email. Nguyên tắc: mỗi khán giả một serializer, đừng tái dùng serializer nội bộ rồi cố `exclude` — sót 1 field là rò dữ liệu; test chốt bằng `leaked.isdisjoint(results[0].keys())`.
  - **`Q` object cho OR trong filter:** deck clone được = `Q(is_starter=True) | Q(visibility=PUBLIC)` — filter kwargs thường chỉ AND được; OR phải qua `Q`. Vẫn giữ nguyên tắc §9: ngoài tập đó là 404, không phân biệt "không tồn tại" với "private của người khác".
  - **`useInfiniteQuery` (TanStack) cho danh sách phân trang:** khác `useQuery`, data là `{pages: [...]}`, kèm `fetchNextPage`/`hasNextPage`; `getNextPageParam` đọc `next` của DRF để biết còn trang không. Trang share cần nó vì deck public có thể 600 từ (12 trang × 50).
- Đọc: `apps/vocab/views.py` (`PublicDeckView`, `PublicDeckWordListView`, `DeckCloneView`), `apps/vocab/serializers.py` (2 serializer public), `apps/vocab/tests/test_share_api.py`, `frontend/src/app/share/[id]/page.tsx`, `frontend/src/app/decks/[id]/page.tsx` (`SharePanel`).
